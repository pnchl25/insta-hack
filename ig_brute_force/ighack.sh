...
banner(){
clear
if [ -f "$HOME/insta-hack/ig_brute_force/setup" ];
then
cd ig_brute_force
bash setup
else
...
echo -e "\e[91mTOR\e[96m IS NOT RUNNING, PLEASE START TOR IN A NEW SESSION
          BY TYPING\e[92m tor\e[96m AND START ig_brute_force AGAIN\e[0m "
exit 1
fi
vid
##############3
...
vid(){
FILE=$HOME/insta-hack/ig_brute_force/noob.noob
if [ -f "$FILE" ]; then
pop
else
echo
